...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](http://ietibackendaguilera.centralus.azurecontainer.io:8080/users)
...
# Local use with backend

1. Clone the repository

2. Build the project with the `gradle build` command.

3. Build the docker image with the command `docker build -t <image name>. `.


4. Ejecutar una instancia con la imagen usando el comando `docker run -p <port>:8080 <name of the image> `.
 
5. Go to `http://localhost:<port>/users`.

6. You will get the following result with the information of the users

   ![](img/app1.PNG)

# Local Use With Graphical Interface
...
4. Execute the app with the command `npm start`.

5. You will get the following result accessing the url `http://localhost:3000`
 
   ![](img/app2.PNG)

# Remote Use With Dockerhub

1. Run the DockerHub image with the command `docker run -d -p <port>: 8080 --name <image name> nicolasaguilera9906/ieti-backend`.

2. Go to the URL `http://localhost:<port>/users`.

# Remote Use In Azure

1. Go to the URL  `http://ietibackendaguilera.centralus.azurecontainer.io:8080/users`.
