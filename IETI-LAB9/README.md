...
- How many customers were created in the database?
  ...
  5 customers were created in the database

- Where is the *findAll* method implemented?

  The method findAll() is implemented in the CrudRepository class, this method returns all instances of the type.
...
- Suppose you have more than 1000 products in your database. How would you implement a method for supporting pagination and return pages of 50 products to your frontend?
  ...
  With the Pageable class we can obtain a certain number of records passing as a parameter to the constructor the number of the page and the size that we want to obtain in it.

- How many products contain the "plus" word in their description?

  4 products have the "plus" word in their description.
 
- How many products are returned by the *findByDescriptionContaining* query? Why?

  There are 4 products that contain the word "plus" in their description. However, in this case only 2 are returned because the second parameter of the PageRequest of () method   requests the size of the page.
  
- Which are the collection names where the objects are stored? Where are those names assigned?
  The name is obtained when the @Document notation is added to the class that will be persisted in the database. 
...
8. Implement the queries of the previous step using *derived query methods* in your repository interface. Is it possible to implement all of them?
   ...
   Is not possible to implement the query : Users that have assigned more than 2 Todos.
